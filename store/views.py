...
def success(request):
    return render(request, 'store/successs.html')

def cart_view(request):
    cart = Cart(request)

    return render(request, 'store/cart_views.html', {
        'cart':cart
    })
...
def search(request):
    ...
    return render(request, 'store/searchs.html', {
        'query':query,
        'products':products
    })
...
def category_detail(request, slug):
    ...
    products = category.products.filter(status=Product.ACTIVE).all()
